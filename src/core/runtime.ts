...
export class SimulationRuntime {
  ...
  constructor(twins: Twin[] = []) {
    for (const t of twins) this.add(t);
  }

  add(t: Twin) {
    if (this.registry.has(t.state.id)) throw new Error(`Duplicate twin ${t.state.id}`);
  ...
  emit<T extends Record<string, unknown>>(e: Omit<SimEvent<T>, "id" | "time">) {
  ...
  step(dt: number) {
    ...
    for (const t of [...this.registry.values()]) {
      if (t.state.active) t.tick(dt, c);
    }
    this.time += dt;
    this.drainEvents();
  }

  run(duration: number, dt = 0.25) {
  ...
  clone() {
    ...
    for (const e of this.history) x.history.push(structuredClone(e));
    for (const e of this.queue) x.queue.push(structuredClone(e));
    return x;
  }
  ...
  private materialize(e: SimEvent) {
