...
const physical = (material, properties) => ({
...
export class WeatherTwin extends BaseTwin {
    windX;
    windZ;
    constructor(id, windX = 2, windZ = 0) {
        super({ id, kind: "weather", position: { x: 0, y: 0, z: 0 }, fidelity: 1, active: true, integrity: 1, temperatureK: 303, metadata: { windX, windZ } }, physical("atmosphere", { windX, windZ }));
        this.windX = windX;
        this.windZ = windZ;
    }
    onEvent() { }
...
export class PipeTwin extends BaseTwin {
    ...
    failed = false;
    connectedValveId;
    constructor(id, position, chemical = "propane", lengthM = 8, diameterMm = 150) {
        super({ id, kind: "pipe", position, fidelity: 1, active: true, integrity: 1, temperatureK: 303, metadata: { chemical, lengthM, diameterMm, flowRateKgS: 5.0, leakRateKgS: 0 } }, physical("carbon_steel", { chemical, lengthM, diameterMm, nominalPressureBar: 25 }));
    ...
    onEvent(event, context) {
        ...
        if (event.type === "thermal.exposure" && event.targetId === this.state.id) {
            ...
            if (!this.failed && (this.state.integrity <= 0.55 || this.state.temperatureK >= 390)) {
                ...
                const nearbyFire = [...context.twins()].find(t => t.state.kind === "fire" && t.state.active &&
                    Math.hypot(t.state.position.x - this.state.position.x, t.state.position.z - this.state.position.z) < 12);
                if (nearbyFire) {
                    context.emit({
                        type: "fire.created",
        ...
        if (event.type === "blast.impact" && event.targetId === this.state.id) {
            ...
            if (overpressureKpa > 40) {
                ...
                if (!this.failed && this.state.integrity <= 0.4) {
                    ...
                    const nearbyFire = [...context.twins()].find(t => t.state.kind === "fire" && t.state.active &&
                        Math.hypot(t.state.position.x - this.state.position.x, t.state.position.z - this.state.position.z) < 20);
                    if (nearbyFire) {
                        context.emit({
                            type: "fire.created",
                            sourceId: this.state.id,
...
export class TankTwin extends BaseTwin {
    ...
    failed = false;
    pressureBar = 15;
    constructor(id, position, chemical = "propane", tankGeometry = "bullet", capacityM3 = 100) {
        super({
            id,
            kind: "tank",
            position,
    ...
    onEvent(event, context) {
        ...
        if (event.type !== "thermal.exposure" || event.targetId !== this.state.id || this.failed)
            return;
        ...
        if (this.heatDose >= 900 || this.state.temperatureK >= 520 || this.state.integrity <= 0.65) {
            ...
            for (const t of context.twins()) {
                if (t.state.id === this.state.id || t.state.kind !== "tank")
                    continue;
                const dist = Math.hypot(t.state.position.x - this.state.position.x, t.state.position.z - this.state.position.z);
                if (dist < 20) {
...
export class ValveTwin extends BaseTwin {
    ...
    openFraction = 1.0;
    failed = false;
    constructor(id, position, valveType = "esdv", connectedPipeId) {
        super({
            id,
            kind: "valve",
            position,
        ...
        this.connectedPipeId = connectedPipeId;
    }
    onEvent(event, context) {
        if (event.sourceId === this.state.id)
            return;
...
export class PumpTwin extends BaseTwin {
    ...
    vibrationMmS = 1.2;
    failed = false;
    constructor(id, position, pumpType = "centrifugal_feed", flowRateM3H = 85) {
        super({
            id,
            kind: "pump",
            position,
...
export class SuppressionTwin extends BaseTwin {
    ...
    agent;
    isDeluging = false;
    constructor(id, position, coverageRadiusM = 14, flowRateKgS = 30, agent = "water_deluge") {
        super({
            id,
            kind: "suppression",
            position,
    ...
    onEvent(event, context) {
        ...
        if (event.sourceId.startsWith('DELUGE') || event.sourceId.startsWith('suppression'))
            return;
...
export class WorkerTwin extends BaseTwin {
    ...
    targetPos;
    evacuated = false;
    constructor(id, position, role = "Field Operator", musterPoint = { x: 30, y: 0, z: 20 }) {
        super({
            id,
            kind: "worker",
            position,
...
export class FlareStackTwin extends BaseTwin {
    ...
    pilotActive;
    flaringRateKgS = 0;
    constructor(id, position, stackHeightM = 22, pilotActive = true) {
        super({
            id,
            kind: "flare",
            position,
...
export class IgnitionSourceTwin extends BaseTwin {
    enabled;
    constructor(id, position, enabled = true) {
        super({ id, kind: "ignition", position, fidelity: 1, active: true, integrity: 1, temperatureK: 650, metadata: { enabled } }, physical("ignition-source", { enabled }));
...
export class WallTwin extends BaseTwin {
    widthM;
    heightM;
    constructor(id, position, widthM = 8, heightM = 4) {
        super({
            id,
            kind: "wall",
            position,
