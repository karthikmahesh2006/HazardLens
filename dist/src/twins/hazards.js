...
export class ReleaseTwin extends BaseTwin {
    ...
    age = 0;
    ignited = false;
    constructor(id, p, sourceId, rateKgS) {
        super({
            id,
            kind: "release",
            position: { ...p },
    ...
    tick(dt, c) {
        ...
        this.age += dt;
        // Wind drift
        const w = [...c.twins()].find(t => t instanceof WeatherTwin);
        const wx = w?.windX ?? 0;
        const wz = w?.windZ ?? 0;
...
export class FireTwin extends BaseTwin {
    intensityMw;
    fireAge = 0;
    contactTime = new Map(); // asset id → seconds of direct contact
    spawned = new Set(); // already cascaded to this asset
    constructor(id, p, intensityMw) {
        super({
            id,
            kind: "fire",
            position: { ...p },
        ...
        this.intensityMw = intensityMw;
    }
    onEvent(e) {
        if (e.type === "suppression.command") {
            const strength = Number(e.payload.strength ?? 0.5);
    ...
    tick(dt, c) {
        if (!this.state.active || this.intensityMw <= 0)
            return;
        this.fireAge += dt;
        ...
        for (const t of c.twins()) {
            if (t.state.id === this.state.id)
                continue;
            if (!["tank", "wall", "pipe", "valve", "pump", "worker", "suppression"].includes(t.state.kind))
                continue;
...
export class BlastTwin extends BaseTwin {
    ...
    shockIgnited = new Set(); // clouds already shock-ignited
    constructor(id, p, energyMj = 1000, yieldKgTnt = 250) {
        super({
            id,
            kind: "blast",
            position: { ...p },
