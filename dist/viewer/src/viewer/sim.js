...
export class ViewerSimulation {
    ...
    scrubTo(timeSec) {
        if (!this.historySnapshots.length)
            return undefined;
    ...
    update(realDt) {
        if (!this.running || this.scrubbedSnapshot)
            return;
    ...
    evaluateCounterfactual(interventionName, applyIntervention, horizonSec = 45) {
        ...
        const calcCost = (snap) => {
            ...
            if (snap.twins.some(t => t.kind === 'blast'))
                cost += 6.0;
            return cost;
        };
